...
export function streamChat(
  ...
  fetch('/api/chat/stream', {
    ...
    .then(async (res) => {
      ...
      const fireDone = () => {
        if (!doneCalled) { doneCalled = true; callbacks.onDone() }
      }
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          console.log('[SSE] event:', eventLine, '| data:', dataLine.slice(0, 80))

          if (eventLine === 'text')  { try { callbacks.onText(JSON.parse(dataLine)) } catch { callbacks.onText(dataLine) } }
          if (eventLine === 'data') {
            try {
          ...
          if (eventLine === 'done')  { console.log('[SSE] calling onDone'); fireDone() }
        }
      }
      ...
      // Guarantee onDone is always called when stream ends
      fireDone()
    })
    .catch((e) => {
